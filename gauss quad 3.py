...
def Herm(n,x): #generating hermite polynomial by series summation
    h = 0
    if(n % 2 == 0):
        l = int(n/2)
    else:
...
def H(n,x): #generating legendre polynomial by recurrsion relation
    if(n == 0):
        return 1 
    elif(n == 1):
        return 2*x 
    else:
        return 2*x*H(n-1,x)-(2*n-2)*H(n-2,x)
